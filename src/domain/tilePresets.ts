...
export const tileBounds = {
  ...
  shadowOpacity: [0, 0.18], shadowSoftness: [8, 40], shadowDepth: [0, 18], hoverGlowIntensity: [0, 0.24], selectedGlowIntensity: [0, 0.30],
...
export function normalizeTileSettings(settings: TileAppearanceSettings): TileAppearanceSettings {
  ...
  return {
    ...
    gap: clamp(settings.gap, tileBounds.gap),
    radius: clamp(settings.radius, tileBounds.radius),
    iconSize: clamp(settings.iconSize, tileBounds.iconSize),
    glassOpacity: clamp(settings.glassOpacity, tileBounds.glassOpacity),
    blur: clamp(settings.blur, tileBounds.blur),
    saturation: clamp(settings.saturation, tileBounds.saturation),
    borderOpacity: clamp(settings.borderOpacity, tileBounds.borderOpacity),
    shadowOpacity: clamp(settings.shadowOpacity, tileBounds.shadowOpacity),
    shadowSoftness: clamp(settings.shadowSoftness, tileBounds.shadowSoftness),
    shadowDepth: clamp(settings.shadowDepth, tileBounds.shadowDepth),
    hoverGlowIntensity: clamp(settings.hoverGlowIntensity, tileBounds.hoverGlowIntensity),
    selectedGlowIntensity: clamp(settings.selectedGlowIntensity, tileBounds.selectedGlowIntensity),
    hoverLift: clamp(settings.hoverLift, tileBounds.hoverLift),
    hoverScale: clamp(settings.hoverScale, tileBounds.hoverScale),
    pressedScale: clamp(settings.pressedScale, tileBounds.pressedScale),
    transitionMs: clamp(settings.transitionMs, tileBounds.transitionMs),
  };
}
