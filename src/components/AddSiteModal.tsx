...
export function AddSiteModal({onClose,onSave,existing,categories,allowRemoteMetadata=false,allowRemotePreview=true}:{onClose:()=>void;onSave:(site:Site)=>void;existing?:Site;categories:string[];allowRemoteMetadata?:boolean;allowRemotePreview?:boolean}){
  ...
  const submit=(e:React.FormEvent)=>{e.preventDefault();const clean=domain.replace(/^https?:\/\//,'').split('/')[0];if(!title.trim()||!clean)return;onSave({...existing,title:title.trim(),domain:clean,desc:desc.trim()||'Сохранённый сайт',note:note.trim(),tags:tags.split(',').map(tag=>tag.trim()).filter(Boolean),color,icon:title.trim()[0].toUpperCase(),category,screenshotUrl:existing?.screenshotUrl||(allowRemotePreview?`https://image.thum.io/get/width/900/crop/420/https://${clean}`:undefined)})};
