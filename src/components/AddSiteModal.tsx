...
export function AddSiteModal({onClose,onSave,existing,categories,groups=[],allowRemoteMetadata=false,allowRemotePreview=true}:{onClose:()=>void;onSave:(site:Site)=>void;existing?:Site;categories:{id:string;name:string}[];groups?:{id:string;name:string;categoryId:string}[];allowRemoteMetadata?:boolean;allowRemotePreview?:boolean}){
  ...
  const [error,setError]=useState('');const submit=(e:React.FormEvent)=>{e.preventDefault();const clean=domain.trim().replace(/^[a-z][a-z\d+.-]*:\/\//i,'').split('/')[0];if(!title.trim()||!clean)return;if(!/^(?=.*[.:])[\p{L}\d-]+(\.[\p{L}\d-]+)*(:\d{1,5})?$/u.test(clean)){setError('Адрес должен выглядеть как example.com или localhost:3000');return}setError('');onSave({...existing,id:existing?.id||`site-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,title:title.trim(),domain:clean,desc:desc.trim()||'Сохранённый сайт',note:note.trim(),tags:tags.split(',').map(tag=>tag.trim()).filter(Boolean),color,icon:title.trim()[0].toUpperCase(),categoryId,groupId:groupId||undefined,category:categories.find(item=>item.id===categoryId)?.name||existing?.category||'Без категории',screenshotUrl:existing?.screenshotUrl||(allowRemotePreview?`https://image.thum.io/get/width/900/crop/420/https://${clean}`:undefined)})};
