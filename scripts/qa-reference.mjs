...
export async function runReferenceQa(root) {
  ...
  const [shell, calendarCss, types, settings, bridge, shellCss, gridCss, tokens] = await Promise.all([
    source(root, 'src/components/shell/AppShell.tsx'), source(root, 'src/components/calendar/CalendarPopover.module.css'),
    source(root, 'src/domain/types.ts'), source(root, 'src/components/settings/TileSettingsPanel.tsx'),
    source(root, 'src/state/TileStyleBridge.tsx'), source(root, 'src/components/shell/AppShell.module.css'),
    source(root, 'src/components/tiles/SpeedDialGrid.module.css'), source(root, 'src/styles/tokens.css'),
  ]);
