...
export async function runDataQa(root) {
  ...
  add(checks, failures, 'backup-preserves-preferences', backup.includes('preferences?: UserPreferences') && backup.includes('isPreferences') && dataControls.includes('preferences') && store.includes('snapshot.preferences') && store.includes('storage.set(KEYS.preferences, preferences)'), 'Export/import must preserve user preferences while accepting old backups');
  add(checks, failures, 'spaces-migrate-legacy-projects', store.includes("spaces: 'nexus.spaces'") && store.includes("projects: 'nexus.projects'") && store.includes('normalizeSpaces(persistedSpaces ?? legacyProjects)'), 'Store must migrate legacy projects into canonical spaces without deleting data');
  add(checks, failures, 'navigation-persists-canonically', store.includes("navigation: 'nexus.navigation'") && ['activeSpaceId','activeCategoryId','contentMode','layoutMode'].every(token => store.includes(token)) && !['workspaceTab:','activeProjectId:','bookmarkQuery:','setSection('].some(token => store.includes(token)), 'Canonical navigation state must persist without deprecated UI adapters');
  ...
  add(checks, failures, 'recent-mode-uses-history', workspace.includes("input.mode === 'recent'") && workspace.includes('input.history') && siteTile.includes('recordVisit(site.id)'), 'Recent mode must derive from persisted visit history and site opens must record visits');
  ...
  add(checks, failures, 'backup-export-derives-legacy-project-shape', dataControls.includes('const projects = spaces.map') && dataControls.includes('createBackup({ preferences, tileSettings, projects'), 'Backup v1 export may derive legacy projects from canonical spaces without keeping projects in runtime state');
